...
3. Then we rename the POSCARs and put into subfolders, command: `parallel "mkdir {} && mv POSCAR-{} {}/POSCAR" ::: {001..xyz}` We also need to copy all the other input files (INCAR, POTCAR, KPOINTS, and run file) into each folder, `parallel "cd {} && cp ../run ." ::: {001..nnn}`.
